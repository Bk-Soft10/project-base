...
odoo.define('allure_backend_theme.Menu', function (require) {
    ...
    SystrayMenu.include({
        willStart: function () {
            var self = this;
            if (allureThemeConfigs.base_menu !== 'base_menu' && _.contains(odoo._modules, 'sync_global_search')) {
                var GlobalSearchSystrayMenu;
                for (var i = SystrayMenu.Items.length - 1; i >= 0; i--) {
                    if (SystrayMenu.Items[i].prototype.template === "GlobalSearch") {
                        GlobalSearchSystrayMenu = SystrayMenu.Items[i];
                        break;
                    };
                };
                SystrayMenu.Items = _.without(SystrayMenu.Items, GlobalSearchSystrayMenu);
            }
    ...
    Menu.include({
        ...
        init: function (parent, menu_data) {
            ...
            this.menu_id = true;
            this.themeData = allureThemeConfigs;
        },
        start: function () {
            var self = this;
        ...
        _loadQuickMenu: function () {
            ...
            if (self.themeData.base_menu !== 'base_menu' && _.contains(odoo._modules, 'sync_global_search') && _.has(odoo.__DEBUG__.services, 'sync_global_search.global_search')) {
                new odoo.__DEBUG__.services['sync_global_search.global_search'](self).appendTo(self.$('.o_quick_menu'));
            };
        },
    });
});