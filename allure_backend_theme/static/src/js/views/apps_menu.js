odoo.define('allure_backend_theme.AppsMenu', function (require) {
    ...
    AppsMenu.include({
        events: _.extend({}, AppsMenu.prototype.events, {
            ...
            'click a.menu-item': '_onClickMenuItem',
        }),
        ...
        start: function () {
            ...
            return $.when(_Super, this._loadMenuItems());
        },

        _loadMenuItems: function() {
            var self = this;
            return this._rpc({
                route: '/all/menu/search',
                params: {}
            }).then(function(menus) { self.menu_items = menus; });
        },
        ...
        _onChange: function (e) {
            ...
            if (input_val) {
                var search_menus = fuzzy.filter(input_val, _.pluck(self.menu_items, 'name'));
                var result_menu = _.map(search_menus, function (result) {
                    return self.menu_items[result.index];
                });
                _.each(result_menu, function (menu) {
                    menu_items.push({
                        action: parseInt(menu.action.split(',')[1]),
                        id: menu.id,
                        name: menu.name,
                        complete_name: menu.complete_name.split('/').slice(0, -1).join(' / '),
                        parentID: menu.parent_path.split('/')[0],
                    });
                });
            } else {
                menu_items = [];
            }
            self.$el.find('.oe_apps_menu').html(QWeb.render('AppsMenuFilter', {
                widget: self,
        ...
        _onClickMenuItem: function(e) {
            ...
            this.select_item($itemLink.data('menu-id'), $itemLink.data('action-id'), $itemLink.data('parent-id'));
        },
