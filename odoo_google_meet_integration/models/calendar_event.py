...
class CalendarEvent(models.Model):
    ...
    def _create_google_meet(self, cal_event):
        ...
        if result.get('hangoutLink'):
            ...
            error_result = result.get('error', False) if result else False
            error_msg = error_result.get('message', False) if error_result else False
            msg_error = "Failed to create event : %s" % (str(error_msg) if error_msg else 'NoN')
