...
class analytic_account_budget_report(models.AbstractModel):
    _name = 'report.om_account_budget.report_analyticaccountbudget'
    ...
    def funct(self, object, form, ids=None, done=None, level=1):
        ...
        result = []
        accounts = self.env['account.analytic.account'].browse([object.id])
        c_b_lines_obj = self.env['crossovered.budget.lines']
        obj_c_budget = self.env['crossovered.budget']
        ...
        for account_id in accounts:
            ...
            self._cr.execute('SELECT DISTINCT(crossovered_budget_id) FROM crossovered_budget_lines WHERE id =ANY(%s)',(b_line_ids,))
            budget_ids = self._cr.fetchall()
